...
**簡略化のポイント**:
- `type` と `file` オプションを削除（プロジェクト配下を全ダウンロード）
- プロジェクト指定なしの場合は `"default"` を使用
...
#### getProjectFiles(owner, repo, project)

**目的**: GitHub API を使用してプロジェクト配下のファイルリストを再帰的に取得

**シグネチャ**:

```javascript
async function getProjectFiles(owner, repo, project) {
  // ...
}
```
...
- `Promise<string[]>`: ファイルパスのリスト
  ```javascript
  [
    "projects/default/.github/chatmodes/file1.md",
    "projects/default/.claude/instructions.md",
    "projects/default/CLAUDE.md"
  ]
  ```
...
1. GitHub API エンドポイントを構築
   ```
   https://api.github.com/repos/{owner}/{repo}/contents/projects/{project}
   ```
2. API リクエストを実行
3. レスポンスを解析
...
**エラー処理**:
- 404 エラー: プロジェクトが存在しない
- 403 エラー: レート制限超過
- その他の HTTP エラー: ネットワークエラー

### 3.3 コア関数

#### downloadFile(filePath, project, force)
...
```javascript
async function downloadFile(filePath, project, force) {
  // ...
}
```

**引数**:
- `filePath` (string): GitHub 上のファイルパス（例: `projects/default/.github/chatmodes/file.md`）
- `project` (string): プロジェクト名（パス変換用）
...
1. **URL 生成**
   ...
   // 例: https://raw.githubusercontent.com/maru3460/airc/main/projects/default/.github/chatmodes/file.md
   ```

2. **ローカルパス生成**
   ```javascript
   // projects/{project}/ を除去
   const localPath = filePath.replace(`projects/${project}/`, "");
   // 例: projects/default/.github/chatmodes/file.md → .github/chatmodes/file.md
   ```
...
3. **GitHub API でファイルリスト取得**
   ```javascript
   const files = await getProjectFiles(REPO_OWNER, REPO_NAME, project);
   ```
   - プロジェクトが存在しない場合はエラー（404）

4. **ダウンロード実行**
   - 各ファイルに対して `downloadFile()` を呼び出し
   ```javascript
   for (const filePath of files) {
     await downloadFile(filePath, project, force);
   }
   ```
...
| オプション  | 短縮形 | 説明                 | デフォルト |
| ----------- | ------ | -------------------- | ---------- |
| `--project` | `-p`   | プロジェクト名を指定 | `default`  |
| `--force`   | `-f`   | 確認なしで上書き     | `false`    |
| `--help`    | `-h`   | ヘルプメッセージ表示 | -          |

**簡略化されたオプション**:
- `--type` と `--file` は削除（プロジェクト配下を全ダウンロード）
- `--list-projects` は削除（GitHub API で動的に取得するため）
...
const ERROR_MESSAGES = {
  PROJECT_NOT_FOUND: (name) =>
    `❌ Project not found: ${name}\n\nPlease check if the project exists in the repository.`,
...
- プロジェクト配下のディレクトリ構造を再帰的に取得するため、複数の API リクエストが必要
- 通常のプロジェクト（3〜5 ディレクトリ）であれば、5〜10 リクエスト程度
- 60 リクエスト/時間で十分対応可能
