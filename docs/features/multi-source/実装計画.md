...
**成果物**:
- `airc add company github:org/repo -p project --token-env GITHUB_TOKEN`
- `airc add company github:org/repo -p project --token ghp_xxxxx`
- `airc add company github:org/repo -p project` (対話的入力)
...
# Phase 1-5 を通じてサポート
airc                  # → デフォルトソースを同期
airc -p project       # → maru3460/airc/{project} を一時的に同期
