...
airc add company github:my-org/ai-prompts -p shared

# Token の指定（3つの方法から選択）

# 方法1: コマンドラインで直接指定
airc add company github:my-org/ai-prompts -p shared --token ghp_xxxxxxxxxxxxx

# 方法2: 環境変数を指定
airc add company github:my-org/ai-prompts -p shared --token-env GITHUB_TOKEN
...
# 出力例:
# Sources:
#   * default (maru3460/airc, project: default)
#   * company (my-org/ai-prompts, project: shared) [authenticated]
...
```bash
# 社内リポジトリを登録
airc add team github:acme-corp/ai-team-config -p engineering --token-env GITHUB_TOKEN
...
```bash
# 個人用（公開リポジトリ）
airc add personal github:myname/ai-config -p personal

# 社内用（private リポジトリ）
airc add work github:company/ai-config -p internal --token-env WORK_GITHUB_TOKEN

# OSS 用（公開リポジトリ）
airc add oss github:myname/ai-config -p oss
```
...
# リモートのリポジトリも登録
airc add remote github:team/ai-config -p production --token-env GITHUB_TOKEN
```
...
```bash
# 公開設定（デフォルト）
airc add public github:maru3460/airc -p default
airc set-default public

# 社内設定
airc add internal github:company/ai-config -p confidential --token-env GITHUB_TOKEN
```
...
cd ~/projects/public-app
airc sync  # → public から同期

# 機密プロジェクト: 社内設定を使用
cd ~/projects/confidential-app
...
# オプション:
#   -p, --project <name>    プロジェクト名（GitHub の場合）
...
# 既存の使い方もサポート
airc              # → デフォルトソースを同期
airc -p project   # → maru3460/airc/{project} を同期（一時的）
